...
void RK4(Vec2D& u, shared_ptr<SysCalcs> sysPtr, Mesh2D& mesh, double& dt, SourceFuncEx sourceFunc, BCFunc BC){
    ...
    for (int i = 1; i < mesh.nCellsX+1; i++){
        for (int j = 1; j < mesh.nCellsY+1; j++){
            for (int var = 0; var < cellVarsNums; var++){
                inner[i][j][var] = u[i][j][var] + dt * (k1[i][j][var] / 2);
            }
        }
    }
    ...
    for (int i = 1; i < mesh.nCellsX+1; i++){
        for (int j = 1; j < mesh.nCellsY+1; j++){
            for (int var = 0; var < cellVarsNums; var++){
                inner[i][j][var] = u[i][j][var] + dt * (k2[i][j][var] / 2);
            }
        }
    }

    BC(inner, mesh, sysPtr);
    Vec2D k3 = sourceFunc(inner, mesh, sysPtr, BC);

    // making k4 term
    for (int i = 1; i < mesh.nCellsX+1; i++){
        for (int j = 1; j < mesh.nCellsY+1; j++){
            for (int var = 0; var < cellVarsNums; var++){
                inner[i][j][var] = u[i][j][var] + dt * k3[i][j][var];
            }
        }
    }

    BC(inner, mesh, sysPtr);
    Vec2D k4 = sourceFunc(inner, mesh, sysPtr, BC);
    ...
    for (int i = 1; i < mesh.nCellsX+1; i++){
        for (int j = 1; j < mesh.nCellsY+1; j++){
            for (int var = 0; var < cellVarsNums; var++){
                u[i][j][var] = u[i][j][var] + dt/6 * (k1[i][j][var] + 2*k2[i][j][var] + 2*k3[i][j][var] + k4[i][j][var]);
            }
        }
    }
